@extends('layouts.app')
@section('title', 'Modifier Groupe — MiniFoot Academy')
@section('page-title', 'Modifier un Groupe')
...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-lg font-bold text-slate-800">{{ $groupe->nom }}</h2>
                        <p class="text-sm text-slate-500 mt-0.5">Modifier les informations du groupe</p>
                    </div>
                </div>
        ...
        <form method="POST" action="{{ route('admin.groupes.update', $groupe) }}" class="p-6">
            @csrf @method('PUT')

            <div class="grid grid-cols-1 gap-5 mb-5">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Nom</label>
                    <input type="text" name="nom" value="{{ old('nom', $groupe->nom) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 transition-all" required>
                ...
                <div>
                    ...
                    <select name="categorie_id" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 transition-all" required>
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ $groupe->categorie_id == $cat->id ? 'selected' : '' }}>{{ $cat->nom }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Coach responsable</label>
                    <select name="coach_id" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 transition-all">
                        ...
                        @foreach($coachs as $coach)
                            <option value="{{ $coach->id }}" {{ $groupe->coach_id == $coach->id ? 'selected' : '' }}>{{ $coach->prenom }} {{ $coach->nom }}</option>
                        @endforeach
                    </select>
    ...
    <form action="{{ route('admin.groupes.destroy', $groupe) }}" method="POST" class="mt-4 text-center">
        @csrf @method('DELETE')
        <button type="submit" onclick="return confirm('Vraiment supprimer ce groupe ?')" class="text-red-500 hover:text-red-700 text-sm font-medium transition-colors">
            <i class="fas fa-trash-alt mr-1"></i> Supprimer définitivement ce groupe
        </button>
    </form>
</div>
@endsection
