@extends('layouts.app')
@section('title', 'Modifier Joueur — MiniFoot Academy')
@section('page-title', 'Modifier un Joueur')
...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-100 bg-linear-to-r from-slate-50 to-white">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-lg font-bold text-slate-800">{{ $joueur->prenom }} {{ $joueur->nom }}</h2>
                        <p class="text-sm text-slate-500 mt-0.5">Modifier les informations du joueur</p>
                    </div>
                </div>
        ...
        <form method="POST" action="{{ route('admin.joueurs.update', $joueur) }}" class="p-6">
            @csrf @method('PUT')

            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Prénom</label>
                    <input type="text" name="prenom" value="{{ old('prenom', $joueur->prenom) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 transition-all" required>
                ...
                <div>
                    ...
                    <input type="text" name="nom" value="{{ old('nom', $joueur->nom) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 transition-all" required>
            ...
            <div class="grid grid-cols-1 gap-5 mb-5">
                <div>
                    ...
                    <input type="date" name="date_naissance" value="{{ old('date_naissance', $joueur->date_naissance->format('Y-m-d')) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 transition-all" required>
                    @error('date_naissance') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Parent</label>
                    <select name="parent_id" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 transition-all" required>
                        @foreach($parents as $parent)
                            <option value="{{ $parent->id }}" {{ $joueur->parent_id == $parent->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="categorie_id" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 transition-all">
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ $joueur->categorie_id == $cat->id ? 'selected' : '' }}>{{ $cat->nom }}</option>
                        @endforeach
                    </select>
    ...
    <form action="{{ route('admin.joueurs.destroy', $joueur) }}" method="POST" class="mt-4 text-center">
        @csrf @method('DELETE')
        <button type="submit" onclick="return confirm('Vraiment supprimer ce joueur ?')" class="text-red-500 hover:text-red-700 text-sm font-medium transition-colors">
            <i class="fas fa-trash-alt mr-1"></i> Supprimer définitivement ce joueur
        </button>
    </form>
</div>
@endsection
